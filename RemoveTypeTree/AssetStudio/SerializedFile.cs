...
namespace AssetStudio
{
    public class SerializedFile
    {
        ...
        public byte[] headerMeataDataNoTypeTree;

        public byte m_FileEndianess;
        public string unityVersion = "2.5.0f5";  // 没有typetree 的version 会增加多两个字符。 10 50  不知道为啥？
        ...
        public SerializedFile(FileReader reader)
        {
            ...
            {
                header.headerData = new byte[reader.BaseStream.Position];
                reader.BaseStream.Position = 0;
                reader.Read(header.headerData, 0, header.headerData.Length);
                header.m_MetadataSize = reader.ReadUInt32();
            ...
            if (header.m_Version >= SerializedFileFormatVersion.Unknown_7)
            {
                ...
                Console.WriteLine($"unityVersion 看看是不是换行 2：" + unityVersion);
            ...
            long fileHeaderPos = reader.BaseStream.Position;
            reader.BaseStream.Position = 0;
            var bytes = new byte[fileHeaderPos];
            reader.BaseStream.Read(bytes, 0, bytes.Length);
            headerMeataDataNoTypeTree = bytes;
            ...
            //read leftData
            long leftDataSize = reader.BaseStream.Length - reader.BaseStream.Position;
            leftFileData = new byte[leftDataSize];
            reader.BaseStream.Read(leftFileData, 0, (int)leftDataSize);
        }

        public void RemoveTypeTree()
        {
            Console.WriteLine($"当前bundle 是否包含typetree: {m_EnableTypeTree} ");
            if (m_EnableTypeTree)
            {
                m_EnableTypeTree = false;

                //很奇怪，no typetree 会多两个字符
                unityVersion += "\n2";
            }
        }
